...
export class AiService {
  ...
  // URLs basées sur les ports configurés : 8082 (Quiz) et 8083 (Training)
  private quizBase  = `${environment.quizApiUrl}/api/quizzes/ai`;
  private trainBase = `${environment.trainingApiUrl}/api/training/ai`;
  ...
  /**
   * 2. RÉSUMÉ MODULE (Port 8083 -> Gemini)
   */
  summarizeModule(moduleId: number | string, language = 'fr'): Observable<ModuleSummaryResponse> {
    return this.http.post<ModuleSummaryResponse>(
      `${this.trainBase}/modules/${moduleId}/summary?language=${language}`, 
      {}
    ).pipe(catchError(this.handleError));
  }

  /**
   * 3. SCRIPT VIDÉO (Port 8083 -> Gemini)
   */
  generateVideoScript(moduleId: number | string, language = 'fr'): Observable<VideoScriptResponse> {
    return this.http.post<VideoScriptResponse>(
      `${this.trainBase}/modules/${moduleId}/video-script?language=${language}`, 
      {}
    ).pipe(catchError(this.handleError));
  }
