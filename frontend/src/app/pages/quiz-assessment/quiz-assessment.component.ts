  ...
  <ng-container *ngIf="viewState()==='MODULE_LIST'">
    <div class="two-col">
      <div class="left-panel" id="guide-module-list">
        <div class="tabs">
          <button class="tab" [class.active]="moduleTab()==='all'"  (click)="moduleTab.set('all')">All ({{ modules().length }})</button>
          <button class="tab" [class.active]="moduleTab()==='done'" (click)="moduleTab.set('done')">Completed</button>
        </div>
        <div class="list">
          <div class="empty" *ngIf="!modules().length && !loading()">No modules available.</div>
...
private mapAttempts(raw: any[]): any[] {
  return raw.map((a: any) => ({
    id:             a.attemptId || a.id,
    date:           a.submittedAt || a.createdAt || new Date(),
  ...
  onSummary(m: any) {
    ...
    this.aiService.summarizeModule(Number(m.id), 'fr').subscribe({
  ...
  onVideoScript(m: any) {
    ...
    this.aiService.generateVideoScript(Number(m.id), 'fr').subscribe({
