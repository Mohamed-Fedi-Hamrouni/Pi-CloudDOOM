...
public interface TrainingModuleRepository extends JpaRepository<TrainingModule, Long> {
    ...
    // ✅ NOUVEAU : charge moduleLessons en une seule requête
    // Utiliser cette méthode dans AiTrainingService à la place de findById()
    @Query("SELECT m FROM TrainingModule m " +
            "LEFT JOIN FETCH m.moduleLessons ml " +
            "WHERE m.id = :id " +
            "ORDER BY ml.orderIndex ASC")
    Optional<TrainingModule> findByIdWithLessons(@Param("id") Long id);

    // ✅ NOUVEAU : charger tous les modules avec leurs leçons (pour listes)
    @Query("SELECT DISTINCT m FROM TrainingModule m " +
            "LEFT JOIN FETCH m.moduleLessons")
    List<TrainingModule> findAllWithLessons();
}
